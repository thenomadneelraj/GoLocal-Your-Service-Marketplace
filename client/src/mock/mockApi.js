...
function randomDelay() {
  const delay = Math.random() * (MOCK_DELAY.max - MOCK_DELAY.min) + MOCK_DELAY.min;
...
export const mockApi = {
  ...
  async signIn(credentials) {
    ...
    // Simulate password check (accept any non-empty password for mock)
  ...
  async createBooking(bookingData) {
    ...
    const provider = mockDB.findOne('users', { _id: service.providerId, role: 'provider', approvalStatus: 'approved' });
    if (!provider) {
  ...
  async sendMessage(messageData) {
    ...
    if (messageData.senderId !== booking.clientId && messageData.senderId !== booking.providerId) {
  ...
  async createDispute(disputeData) {
    ...
    const existingDispute = mockDB.findOne('disputes', { bookingId: disputeData.bookingId });
    if (existingDispute) {
      return createErrorResponse('Dispute already exists for this booking', 409);
    }
...
export function shouldUseMock() {
  ...
  return USE_MOCK || !import.meta.env.VITE_API_URL;
}

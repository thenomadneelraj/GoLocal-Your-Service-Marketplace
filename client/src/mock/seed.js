...
function generateUsers() {
  ...
  // NOTE: Admin user is NOT included in mock data - admin uses real backend
...
function generateBookings(clients, providers, services) {
  ...
  clients.forEach((client) => {
    ...
    for (let i = 0; i < bookingCount; i++) {
      const provider = randomChoice(
        providers.filter((p) => p.approvalStatus === "approved"),
      );
      const service = randomChoice(
        services.filter((s) => s.providerId === provider._id && s.isActive),
      );
      ...
      if (service) {
        ...
        bookings.push({
          ...
          scheduledDate: new Date(
            Date.parse(createdAt) + randomNumber(1, 30) * 24 * 60 * 60 * 1000,
          ).toISOString(),
        });
      }
    }
    ...
    for (let i = 0; i < messageCount; i++) {
      ...
      const createdAt = new Date(
        Date.parse(booking.createdAt) +
          i * randomNumber(1, 24) * 60 * 60 * 1000,
      ).toISOString();

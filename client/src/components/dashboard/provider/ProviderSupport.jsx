...
const STATUS_MAP = {
  ...
  closed: { label: "Closed", color: "bg-slate-500/10 text-slate-600 border-slate-200" },
};
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="bg-card/40 border border-border/60 rounded-[3rem] p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 backdrop-blur-xl relative overflow-hidden group">
        ...
        <div className="relative">
          <h1 className="text-3xl font-semibold tracking-tight">Provider Support</h1>
          <p className="text-muted-foreground mt-2 max-w-md italic leading-relaxed">
            Get help with bookings, payouts, verification, or account issues. Our provider support team is here to assist you.
          </p>
        </div>
        <Button
          size="lg"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-card/40 border border-border/60 rounded-[2rem] p-6 backdrop-blur-xl hover:bg-card/60 transition-all group">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            For detailed inquiries, send us an email and we'll get back to you promptly.
          </p>
          <a
        ...
        <div className="bg-card/40 border border-border/60 rounded-[2rem] p-6 backdrop-blur-xl hover:bg-card/60 transition-all group">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Access our provider knowledge base for best practices and platform guides.
          </p>
          <a
        ...
        <div className="bg-card/40 border border-border/60 rounded-[2rem] p-6 backdrop-blur-xl hover:bg-card/60 transition-all group">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Find quick answers to the most frequently asked questions for providers.
          </p>
          <a
          ...
          <div className="grid gap-4">
            {loading ? (
              ...
              <div className="py-24 flex flex-col items-center justify-center bg-card/30 rounded-[3rem] border border-dashed border-border/60">
                ...
                <h3 className="text-xl font-semibold">No support tickets found</h3>
                      ...
                      <div className="flex items-center gap-4">
                        <span
                          className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${
                            STATUS_MAP[msg.status]?.color || STATUS_MAP.pending.color
                          }`}
                        >
                        ...
                        <span className="text-xs font-bold text-muted-foreground bg-muted/30 px-2 py-0.5 rounded-md italic">
                          {msg.category ? msg.category.toUpperCase() : "GENERAL"}
                        </span>
        ...
        <div className="space-y-6">
          <section className="bg-card/40 border border-border/60 rounded-[2.5rem] p-8 space-y-6">
            ...
            <p className="text-sm text-muted-foreground italic leading-relaxed">
              Our provider support team is available Monday through Friday, 9 AM to 6 PM IST.
            </p>
          ...
          <section className="p-8 bg-primary/5 border border-primary/20 rounded-[2.5rem] flex flex-col gap-4">
            <div className="flex items-center gap-3 text-primary">
              <AlertCircle size={20} />
              <h3 className="font-bold uppercase tracking-widest text-xs">Urgent Issues</h3>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed italic">
              For urgent matters related to active bookings or payouts, please contact us directly at:
            </p>
          ...
          <section className="bg-card/40 border border-border/60 rounded-[2.5rem] p-8">
            ...
            <ul className="space-y-3">
              {[
                { label: "Managing your bookings", link: "/resources/provider-guides" },
                ...
                { label: "Profile optimization", link: "/resources/provider-faqs" },
              ].map((item, i) => (
                <li key={i}>
                  <a
                    href={item.link}
        ...
        <div className="fixed inset-0 z-50 bg-black/55 backdrop-blur-sm flex items-center justify-center px-4">
          <div className="w-full max-w-2xl bg-card rounded-[2rem] border border-border/70 p-8 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-2xl font-bold tracking-tight">Submit Support Ticket</h2>
                <p className="text-sm text-muted-foreground mt-1">
                  Describe your issue and our provider support team will respond within 24 hours.
                </p>
              </div>
              <button
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <Button
                  ...
                  disabled={submittingTicket || !ticketForm.subject || !ticketForm.category || !ticketForm.message}
                >
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  <span
                    className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${
                      STATUS_MAP[selectedTicket.status]?.color || STATUS_MAP.pending.color
                    }`}
                  >
                ...
                <h2 className="text-2xl font-bold tracking-tight">{selectedTicket.subject}</h2>
              </div>
              <button
            ...
            <div className="space-y-6">
              <div className="p-4 bg-muted/30 rounded-xl border border-border/40">
                ...
                <p className="text-sm font-semibold">
                  {selectedTicket.category ? selectedTicket.category.toUpperCase() : "General"}
                </p>
              </div>
